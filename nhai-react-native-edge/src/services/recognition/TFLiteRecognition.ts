...
class FaceRecognitionService {
  ...
  /**
   * Initializes the MobileFaceNet model from the app bundle.
   * Total model size is ~4MB, safely below the 20MB Hackathon limit.
   */
  public async initModel() {
    if (this.isLoaded) return;
    try {
      // Load the .tflite model bundled in the Android assets / iOS bundle
      this.model = await loadTensorflowModel(require('../../../models/mobilefacenet.tflite'));
      this.isLoaded = true;
