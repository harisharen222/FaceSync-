"""
dependencies.py — JWT verification for Enrollment Service.

Instead of duplicating the RS256 verification logic, this service fetches the
Auth Service public key via the JWKS endpoint and caches it in memory.
This means Enrollment Service can verify JWTs independently without calling
...
async def _get_public_key() -> str:
    """
    Fetch the RS256 public key from Auth Service JWKS endpoint.
    Cached in memory — refreshed only on startup or explicit cache invalidation.
    """
    ...
    try:
        ...
        # Convert JWKS n,e → PEM public key
        ...
        logger.info("RS256 public key fetched from Auth Service JWKS ✓")
        return pem
